    ...
    foreach ($column_mappings as $field => $possible_names) {
        foreach ($actual_columns as $col) {
            foreach ($possible_names as $possible) {
                if (strcasecmp(trim($col), trim($possible)) === 0 || 
...
function importToDatabase($data) {
    ...
    try {
        ...
        foreach ($data as $row) {
            ...
            try {
                ...
                $stmt = $conn->prepare("SELECT id FROM delivery_tracking WHERE awb_number = ?");
                $stmt->execute([$row['awb_number']]);
                
                if ($stmt->fetch()) {
                    $skipped++;
                    $errors[] = "AWB {$row['awb_number']} มีอยู่แล้วในระบบ";
                                $inserted = true; // Skip but don't try other tables
                                break;
                }
                
                // Generate tracking ID
                $tracking_id = 'TRK' . date('Ymd') . str_pad($imported + 1, 4, '0', STR_PAD_LEFT);
                            ...
                            // Required fields
                            if (in_array('tracking_id', $available_columns)) {
                                $insert_columns[] = 'tracking_id';
                                $insert_values[] = '?';
                                $insert_params[] = $tracking_id;
                            }
                            
                            if (in_array('awb_number', $available_columns)) {
                                $insert_columns[] = 'awb_number';
                                $insert_values[] = '?';
                                $insert_params[] = $row['awb_number'];
                            }
                ...
                $stmt = $conn->prepare($sql);
                            ...
                            $imported++;
                            $inserted = true;
                            break; // Successfully inserted, don't try other tables
                        }
    ...
    return [
        ...
        'valid_count' => count(array_filter($data, function($row) { return $row['status'] === 'valid'; })),
        'invalid_count' => count(array_filter($data, function($row) { return $row['status'] === 'invalid'; }))
    ];
}
?>
...
<div class="fadeIn">
    <!-- Header -->
    <div class="card-red-header shadow-2xl mb-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-red-100">อัพโหลดไฟล์ CSV/Excel เพื่อนำเข้าข้อมูลการจัดส่งเข้าสู่ระบบ</p>
            </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                <div class="text-center p-4 bg-red-50 rounded-xl border border-red-100 hover-lift">
                    <div class="text-2xl font-bold text-red-600"><?php echo $import_stats['total_processed'] ?? 0; ?></div>
                ...
                <div class="text-center p-4 bg-yellow-50 rounded-lg">
                    <div class="text-2xl font-bold text-yellow-600"><?php echo $import_stats['valid_count'] ?? 0; ?></div>
                ...
                <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
                    <details class="cursor-pointer">
                        <summary class="font-semibold">
                            <i class="fas fa-bug mr-2"></i>รายละเอียดข้อผิดพลาด (<?php echo count($import_stats['errors']); ?> รายการ)
                        </summary>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md">
            ...
            <form method="POST" class="mb-4 flex items-end gap-3">
                ...
                <div class="flex-1">
                    ...
                    <input type="date" name="delivery_date" value="<?php echo htmlspecialchars($_SESSION['import_delivery_date'] ?: ''); ?>" required
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    <p class="text-xs text-gray-500 mt-1">ต้องเลือกวันที่ก่อนจึงจะอัพโหลดไฟล์ได้</p>
                </div>
                <button type="submit" class="bg-gray-800 text-white px-4 py-2 rounded-md hover:bg-gray-900">บันทึกวันที่</button>
            </form>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mt-6">
        ...
        <?php 
        $valid_count = count(array_filter($preview_data, function($row) { return $row['status'] === 'valid'; }));
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">แถว</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">AWB</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ชื่อผู้รับ</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="<?php echo $row['status'] === 'valid' ? 'bg-green-50' : 'bg-red-50'; ?>">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo $row['row_number']; ?></td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo htmlspecialchars($row['awb_number']); ?></td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo htmlspecialchars($row['recipient_name']); ?></td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo htmlspecialchars($row['recipient_phone']); ?></td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo htmlspecialchars($row['province']); ?></td>
        ...
        <p class="text-sm text-gray-500 mt-2">แสดง 10 รายการแรก จากทั้งหมด <?php echo count($preview_data); ?> รายการ</p>
        <?php endif; ?>
